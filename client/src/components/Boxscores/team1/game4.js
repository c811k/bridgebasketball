import React from "react";

function game4(props) {
    return (
        <div className="row mt-5">
            <div className="col-md-12">
                <table className="table table-striped">
                    <thead>
                        <tr className="border-top border-bottom" id="standing-header">
                        <th scope="col">#</th>
                        <th scope="col">PLAYER NAME</th>
                        <th scope="col" className="text-right">FG</th>
                        <th scope="col" className="text-right">3PT</th>
                        <th scope="col" className="text-right">FT</th>
                        <th scope="col" className="text-right">REB</th>
                        <th scope="col" className="text-right">AST</th>
                        <th scope="col" className="text-right">STL</th>
                        <th scope="col" className="text-right">BLK</th>
                        <th scope="col" className="text-right">PF</th>
                        <th scope="col" className="text-right">PTS</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr className="small">
                        <th scope="row">11</th>
                        <td>Caleb Kang</td>
                        <td className="text-right">1-5</td>
                        <td className="text-right">0-2</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">3</td>
                        <td className="text-right">3</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">2</td>
                        </tr>
                        <tr className="small">
                        <th scope="row">20</th>
                        <td>James Choi</td>
                        <td className="text-right">0-3</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">2</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        </tr>
                        <tr className="small">
                        <th scope="row">25</th>
                        <td>James Choi</td>
                        <td className="text-right">5-18</td>
                        <td className="text-right">0-2</td>
                        <td className="text-right">3-6</td>
                        <td className="text-right">11</td>
                        <td className="text-right">2</td>
                        <td className="text-right">2</td>
                        <td className="text-right">1</td>
                        <td className="text-right">2</td>
                        <td className="text-right">13</td>
                        </tr>
                        <tr className="small">
                        <th scope="row">44</th>
                        <td>James Kim</td>
                        <td className="text-right">0-1</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        </tr>
                        <tr className="small">
                        <th scope="row">31</th>
                        <td>Joseph Jung</td>
                        <td className="text-right">1-3</td>
                        <td className="text-right">0-1</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">5</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">1</td>
                        <td className="text-right">2</td>
                        </tr>
                        <tr className="small">
                        <th scope="row">89</th>
                        <td>Moon Kim</td>
                        <td className="text-right">0-4</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">2</td>
                        <td className="text-right">1</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        </tr>
                        <tr className="small">
                        <th scope="row">2</th>
                        <td>Richard Kim</td>
                        <td className="text-right">4-9</td>
                        <td className="text-right">1-5</td>
                        <td className="text-right">0-0</td>
                        <td className="text-right">5</td>
                        <td className="text-right">1</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">0</td>
                        <td className="text-right">9</td>
                        </tr>
                        <tr className="small">
                        <th scope="row">31</th>
                        <td>Smith Kang</td>
                        <td className="text-right">8-19</td>
                        <td className="text-right">3-10</td>
                        <td className="text-right">2-3</td>
                        <td className="text-right">9</td>
                        <td className="text-right">1</td>
                        <td className="text-right">1</td>
                        <td className="text-right">0</td>
                        <td className="text-right">3</td>
                        <td className="text-right">21</td>
                        </tr>
                        <tr className="border-top border-bottom">
                        <th scope="row"><span className="text-weight-bold">Team</span></th>
                        <td></td>
                        <td className="text-right font-weight-bolder">19-62</td>
                        <td className="text-right font-weight-bolder">4-20</td>
                        <td className="text-right font-weight-bolder">5-9</td>
                        <td className="text-right font-weight-bolder">37</td>
                        <td className="text-right font-weight-bolder">8</td>
                        <td className="text-right font-weight-bolder">3</td>
                        <td className="text-right font-weight-bolder">1</td>
                        <td className="text-right font-weight-bolder">6</td>
                        <td className="text-right font-weight-bolder">47</td>
                        </tr>
                        <tr>
                        <th scope="row"></th>
                        <td></td>
                        <td className="text-right font-weight-bolder">30.6%</td>
                        <td className="text-right font-weight-bolder">20.0%</td>
                        <td className="text-right font-weight-bolder">55.6%</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    );
}

export default game4;